...
void disp_set_segment_up() {

}

void disp_set_segment_down() {

}

void disp_set_segment_same() {

}


void convertMessageToDispMsgData(display_message_t message){
	if(message == DISP_MSG_UP)
		disp_set_segment_up();
	else if (message == DISP_MSG_DOWN)
		disp_set_segment_down();
	else
		disp_set_segment_same();
}
...
int disp_show_message(display_message_t message) {
	const int addr = HW_I2C_ADDR_HT16K33;
	convertMessageToDispMsgData(message);
